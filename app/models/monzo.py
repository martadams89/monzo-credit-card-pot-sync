...
class MonzoAccount(db.Model):
    ...
    account_id = db.Column(db.String(100), unique=True, nullable=True)
    ...
    pots = db.relationship('MonzoPot', backref='account', lazy=True)
    
    def __repr__(self):
        return f'<MonzoAccount {self.id}: {self.name}>'

class MonzoPot(db.Model):
    """Model for Monzo pots."""
    ...
    name = db.Column(db.String(100), nullable=False)
    pot_id = db.Column(db.String(100), unique=True, nullable=False)
    balance = db.Column(db.Integer, default=0)
    currency = db.Column(db.String(3), default='GBP')
...
class SyncRule(db.Model):
    """Model for sync rules between accounts and pots."""
    ...
    action_amount = db.Column(db.Integer, nullable=True)
    action_percentage = db.Column(db.Float, nullable=True)
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class SyncHistory(db.Model):
    ...
    def __repr__(self):
        return f'<SyncHistory {self.id}: {self.amount/100} {self.currency}>'
