...
def test_truelayer_account_get_card_balance(requests_mock):
    balance_response = {"results": [{"account_id": "id", "current": 500}]}
    ...
    requests_mock.get("https://api.truelayer.com/data/v1/cards/1/transactions/pending", status_code=200, json=pending_response)

    account = TrueLayerAccount("American Express", "access_token", "refresh_token", time() + 1000)
    assert account.get_card_balance("1") == 515.0
...
def test_truelayer_account_get_total_balance(requests_mock):
    ...
    pending_response_two = {
        "results": [
            {"transaction_id": "txn3", "amount": 20.0, "currency": "GBP"},
            {"transaction_id": "txn4", "amount": 30.0, "currency": "GBP"},
        ]
    }
    requests_mock.get("https://api.truelayer.com/data/v1/cards/2/transactions/pending", status_code=200, json=pending_response_two)

    account = TrueLayerAccount("American Express", "access_token", "refresh_token", time() + 1000)
    # Total balance is 500 + 10 + 5 + 750 + 20 + 30 = 1315 pounds,
    # multiplied by 100 to convert into pence.
    assert account.get_total_balance() == 131500